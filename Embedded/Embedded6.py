...
while (True):
    ...
    if colorMode == 1 and isColored == 1 and isGraped == "0":  # 빨강 색
        ...
        if max_area_r > 10:  # 크기가 80 이상이면 동그라미, 사각형인정
    ...
    if colorMode == 2 and isColored == 1 and isGraped == "0":  # 파란색
        ...
        if max_area_b > 10:  # 크기가 80 이상이면 동그라미, 사각형인정
    ...
    if isGraped == "1":
        ...
        for idx, centroid in enumerate(centroids_y):  # 한 row씩만 뽑아줌
            ...
            centerX_y, centerY_y = int(centroid[0]), int(
                centroid[1])  # centerX, centerY = int(centroid[0]), int(centroid[1])??
            if (area > max_area_y):
                max_area_y = area
                max_centerX_y = centerX_y
                max_centerY_y = centerY_y
        if max_area_y > 80:  # 크기가 80 이상이면 동그라미, 사각형인정
