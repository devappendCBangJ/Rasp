...
try:                                    # 이 try 안의 구문을 먼저 수행하고
    while True:                         # 무한루프 시작: 아두이노의 loop()와 같음
        GPIO.output(LED_pin, GPIO.HIGH) # LED 핀에 HIGH 신호 인가(LED 켜짐) : 아두이노의 digitalwrite와 같음
        time.sleep(1)                   # 1초간 대기 : 아두이노의 delay(1000)과 같음
        GPIO.output(LED_pin, GPIO.LOW)  # LED 핀에 LOW 신호 인가(LED 꺼짐) : 아두이노의 ditialwrite와 같음
        time.sleep(1)                   # 1초간 대기
