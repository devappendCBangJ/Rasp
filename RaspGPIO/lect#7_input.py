...
# 스위치 핀을 풀다운저항이 있는 출력으로 설정 : 아두이노의 pinmode와 같음
# 풀다운 저항이 있으면 버튼을 누르지 않으면 LOW 신호가 됨
# 풀업 저항이 있으면 버튼을 누르지 않으면 HIGH 신호가 됨
...
try:                                    # 이 try 안의 구문을 먼저 수행하고
    while True:                         # 무한루프 시작: 아두이노의 loop()와 같음
        if GPIO.input(sw_pin) == GPIO.HIGH:     # 스위치 핀이 HIGH이면 : 아두이노의 digitalread와 같음
            GPIO.output(LED_pin, GPIO.HIGH)     # LED 핀을 HIGH로(LED 켬)
        else:                                   # 스위치 핀이 HIGH가 아니면 : 아두이노의 digitalread와 같음
